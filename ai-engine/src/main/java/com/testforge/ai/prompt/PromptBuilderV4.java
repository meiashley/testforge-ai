...
public class PromptBuilderV4 implements EndpointPromptBuilder {
    ...
    private static final String FIXTURE_GUIDANCE = """
            TEST FIXTURES AVAILABLE:
            The system pre-creates a seed payment before test execution.
            Available placeholders for HAPPY PATH tests targeting existing data:
            - {{paymentId}}: ID of a pre-existing COMPLETED payment

            For HAPPY PATH GET /api/payments/{id} → use path '/api/payments/{{paymentId}}'
            For HAPPY PATH POST /api/payments/{id}/refund → use path '/api/payments/{{paymentId}}/refund'
            For NEGATIVE/SECURITY tests → still use fake IDs like 'non-existent-id'

            If a path/body field references existing data, use {{paymentId}} placeholder.
            Do not hardcode payment IDs.
            """;
    ...
    private static final String FEW_SHOT_EXAMPLE = """
            ...
            Example of an ideal HAPPY_PATH test case for GET /api/payments/{id} using fixture placeholder:
            {
              "id": "tc-001",
              "name": "Get existing payment by ID",
              "type": "HAPPY_PATH",
              "priority": "P0",
              "scenario": "Given a pre-existing payment When GET /api/payments/{{paymentId}} is called Then a 200 response is returned with full payment details",
              "request": {
                "method": "GET",
              ...
              "reasoning": "Uses {{paymentId}} fixture to reference a real pre-existing payment, avoiding hardcoded IDs."
            }
            """;

    private static final String TEMPLATE = """
            You are an expert API test engineer. Given the following REST API endpoint specification,
            generate test cases that comprehensively cover the endpoint:
            - All response status codes defined in the spec (success + error codes)
            - Happy path scenarios for the main use case
            - Boundary conditions for numeric/string/array fields (min/max/zero/empty)
            - Negative cases: missing required fields, wrong types, invalid values
            - Authorization edge cases if defined in spec
            - Don't repeat similar scenarios; one case per distinct behavior.

            Generate as many cases as needed to cover these dimensions, but avoid
            redundancy. Typical range: 4-10 cases per endpoint depending on complexity.
    ...
    public String build(EndpointSpec spec) {
        ...
        return TEMPLATE
                ...
                .replace("{fixture_guidance}", FIXTURE_GUIDANCE)
