...
AI-powered API quality validation platform. Java 21 + Spring Boot + Anthropic Claude.

## What It Does

Reads an OpenAPI 3.0 specification and requirement document, generates structured API tests and multi-step business scenarios using Anthropic Claude, executes them against the live API, then produces unified execution reports.

Pipeline:
- V4 layer: OpenAPI YAML -> AI Engine (Claude) -> List<TestCase> -> Test Runner -> API execution report
- V5 layer: Requirements + OpenAPI -> AI Engine (Claude) -> Scenario plans -> Test Runner -> Scenario execution + consistency analysis
- Unified output: API results + scenario results + consistency findings + AI diagnoses (JSON + Markdown + HTML)

## 📊 Live Demo Reports

[![V5 Unified Report](https://img.shields.io/badge/🆕_V5_Unified_Report-Live-orange?style=for-the-badge)](https://meiashley.github.io/testforge-ai/sample-v5-execution-report.html)
[![V4 with AI Analysis](https://img.shields.io/badge/🆕_V4_+_AI_Failure_Analyzer-Live-blueviolet?style=for-the-badge)](https://meiashley.github.io/testforge-ai/sample-v4-execution-report.html)
...
**🆕 [V5 unified execution report](https://meiashley.github.io/testforge-ai/sample-v5-execution-report.html)** — combines V4 API-layer generation/execution with V5 requirement-driven scenarios, consistency checking, and AI failure diagnosis in one report. Latest baseline: 34 API tests (28 passed), 6 scenarios (3 passed), 23 mismatch findings, 9 AI diagnoses.

**🆕 [V4 execution report with AI Failure Analyzer](https://meiashley.github.io/testforge-ai/sample-v4-execution-report.html)** — 34 test cases generated by dimension-driven prompts; 6 failures auto-diagnosed by AI with confidence labels (including one **real API bug** discovered: refund on already-REFUNDED payment returns 200 instead of 422).

**[V3.1 baseline report](https://meiashley.github.io/testforge-ai/sample-execution-report.html)** — 9/9 passed with narrow scope, 100% pass rate against real Claude Sonnet 4.5 API.
...
| ai-engine | V5 | OpenAPI test generation + requirement analysis + consistency check + scenario planning |
| test-runner | V5 | API execution + scenario execution + unified HTML/JSON/MD reporting |
...
### Iteration Story: V1 → V4

The project's iteration story shows clear engineering progression — each version targets a different quality dimension:

| Version | Test Cases | Pass Rate | What it proves                                    |
|---------|------------|-----------|---------------------------------------------------|
| V1      | 9          | 55.6%     | Naive baseline; identified state-pollution issues |
| V2      | 9          | 77.8%     | Added state machine context (+22.2pp)             |
| V3      | 9          | 88.9%     | Added fixture lifecycle (+11.1pp)                 |
| V3.1    | 9          | **100%**  | Per-test lazy fixtures (narrow scope, reliable)   |
| V4      | **34**     | 82.4%     | Dimension-driven generation (broad exploration, AI failure diagnosis) |

V3.1 proved narrow-scope reliability; V4 explored generation depth — failed cases auto-diagnosed by the AI Failure Analyzer.
...
Quality is not measured by pass rate alone. TestForge AI validates AI-generated tests on four layers:
...
4. **Failure diagnosis: AI Failure Analyzer** — when tests fail, Claude diagnoses root causes in batch (category + evidence + suggested fix + confidence). Catches issues that pass-rate alone misses: distinguishes test bugs from real API bugs.

This combination catches four different failure modes:
...
- ❌ A failure that looks like a test bug but is a real API bug → caught by AI Failure Analyzer
...
### V5 Baseline — Unified API + Scenario Validation

V5 is not a replacement for V4. It adds a second validation layer: requirement-driven business scenario planning with consistency checking, then merges those results with V4 API-layer execution into a single report.

| Metric                    | V5 Result             |
|---------------------------|-----------------------|
| API tests                 | **34 total / 28 passed / 6 failed** |
| Business scenarios        | **6 total / 3 passed / 3 failed** |
| Consistency mismatch records | **23**            |
| AI diagnoses              | **9**                 |

The V5 report includes:
- API Tests summary (from V4-style OpenAPI-driven generation)
- Scenario execution summary (multi-step business flows)
- Requirement-vs-OpenAPI consistency findings
- AI diagnoses across both API and scenario failures

**[View V5 unified execution report](https://meiashley.github.io/testforge-ai/sample-v5-execution-report.html)** — API-layer results, scenario outcomes, consistency analysis, and AI diagnoses in one artifact.
...
- [x] V5: Requirement-driven scenario planning + consistency check + unified report
- [ ] V5.1: Multi-fixture types and teardown phase
